"""Generate evidence-grounded CV content and deterministic DOCX candidates."""
...
CONFIDENTIAL_PATTERN = re.compile(
    r"\b(?:confidential|proprietary|trade secret|under (?:an )?nda|"
    r"do not disclose)\b",
    flags=re.IGNORECASE,
)
METRIC_PATTERN = re.compile(
    r"(?<![\w])(?:[$£€]\s*)?\d[\d,.]*(?:\s*%|\s*(?:years?|months?|x))?",
    flags=re.IGNORECASE,
)
TITLE_PATTERN = re.compile(
    r"\b(?:[A-Z][\w+#.-]+\s+){0,2}(?:Engineer|Developer|Architect|"
    r"Analyst|Consultant|Manager|Director|Lead|Head|Officer)\b"
)
...
class CvGroundingError(ValueError):
    """Raised when generated CV content is not supported by cited evidence."""
...
def _evidence_text(
    block_ids: tuple[str, ...], blocks: dict[str, SourceBlock]
) -> str:
    return "\n".join(blocks[block_id].content for block_id in block_ids)


def _require_exact_fact(
    value: str, evidence: str, label: str, *, partial: bool = False
) -> None:
    fact = _normalized(value)
    source = _normalized(evidence)
    if not fact or (fact not in source and not (partial and source in fact)):
...
def _validate_confidential_text(content: CvContent) -> None:
    ...
    if any(CONFIDENTIAL_PATTERN.search(value) for value in text_values):
        raise CvGroundingError(
            "Generated CV contains confidential or restricted wording."
        )


def _validate_claim_facts(
    section_kind: CvSectionKind, claim_text: str, evidence: str
) -> None:
    for metric in METRIC_PATTERN.findall(claim_text):
        _require_exact_fact(metric, evidence, "date or metric")
    for title in TITLE_PATTERN.findall(claim_text):
        _require_exact_fact(title, evidence, "professional title")
    if section_kind is CvSectionKind.SKILLS:
        skill_items = re.split(r"[,;|•\n]+|\band\b", claim_text)
        for skill in skill_items:
            cleaned = skill.strip(" .:-")
            if cleaned:
                _require_exact_fact(cleaned, evidence, "skill")
    if section_kind in {
        CvSectionKind.EDUCATION,
        CvSectionKind.CERTIFICATIONS,
    }:
        _require_exact_fact(claim_text, evidence, section_kind.value)


def validate_grounded_cv_content(
    content: CvContent,
    assessment: AssessmentResult,
    profile_blocks: tuple[SourceBlock, ...],
) -> None:
    """Resolve citations and reject unsupported high-risk CV facts."""
    allowed = _allowed_evidence(assessment)
    ...
    for label, value in identity_values:
        ...
        _require_exact_fact(
            value.text,
            _evidence_text(value.profile_block_ids, blocks),
            label,
        )
    ...
    for section in content.sections:
        for claim in section.claims:
            ...
            evidence = _evidence_text(claim.profile_block_ids, blocks)
            _validate_claim_facts(section.kind, claim.text, evidence)


class CvGenerationService:
    """Generate, validate, render, and persist one grounded CV candidate."""
    ...
    def execute(
        ...
        *,
        ...
        """Produce and persist one grounded DOCX candidate."""
        ...
        validate_grounded_cv_content(
            generated.value, assessment, profile_blocks
        )
